...
def parse_file(sheet_key, path):
    ...
    raw_headers = [jsonable(ws.cell(row=sheet_def["header_row"], column=col).value) for col in selected_cols]
    ...
    rows = []
    blank_streak = 0
    print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] Reading {sheet_key}: sheet '{sheet_name}', Excel max row {ws.max_row:,}...", flush=True)
    for source_row in range(sheet_def["data_start_row"], ws.max_row + 1):
        values = [jsonable(ws.cell(row=source_row, column=col).value) for col in selected_cols]
        while len(values) < MAX_COLS:
            values.append("")
        if all(str(v or "").strip() == "" for v in values):
            blank_streak += 1
            if rows and blank_streak >= STOP_AFTER_BLANK_ROWS:
