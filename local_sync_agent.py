...
def make_temp_copy_path(source_path, sheet_key):
    ext = os.path.splitext(source_path)[1] or ".xlsx"
    safe_name = f"slic_{sheet_key.lower()}_{datetime.now():%Y%m%d_%H%M%S}_{uuid.uuid4().hex[:8]}{ext}"
    return os.path.join(tempfile.gettempdir(), safe_name)


def try_filesystem_copy(source_path, sheet_key):
    temp_path = make_temp_copy_path(source_path, sheet_key)
    try:
        shutil.copy2(source_path, temp_path)
        print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: using temporary copy: {temp_path}", flush=True)
        return temp_path
    except Exception as ex:
        print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: normal temp copy failed: {ex}", flush=True)
        return None


def try_excel_save_copy_as(source_path, sheet_key):
    """When the workbook is open in Excel, Python/openpyxl may get Permission denied.
    If Excel already has the workbook open, use Excel's SaveCopyAs to create a readable
    temporary snapshot, then openpyxl reads that copy instead.
    Requires: python -m pip install pywin32
    """
    if os.name != "nt":
        return None
    temp_path = make_temp_copy_path(source_path, sheet_key)
    try:
        ...
        print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: Excel SaveCopyAs fallback needs pywin32. Install with: python -m pip install pywin32", flush=True)
        return None
    ...
    try:
        ...
        target = os.path.abspath(source_path).lower()
        for wb in excel.Workbooks:
            try:
                if os.path.abspath(wb.FullName).lower() == target:
                    wb.SaveCopyAs(temp_path)
                    print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: Excel file is open; created readable snapshot using SaveCopyAs.", flush=True)
                    return temp_path
            except Exception:
                continue

        print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: workbook is locked and not found in this Excel session.", flush=True)
        return None
    except Exception as ex:
        print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: Excel SaveCopyAs fallback failed: {ex}", flush=True)
        return None
    finally:
        try:
...
def open_workbook_with_lock_fallback(source_path, sheet_key):
    """Open workbook directly when possible. If Windows/OneDrive/Excel locks it,
    retry via temporary snapshots so auto sync can keep running while Excel is open.
    Returns (workbook, temp_path_to_delete_or_None).
    """
    errors = []
    for attempt in range(1, 4):
        try:
            return load_workbook(source_path, read_only=True, data_only=True, keep_vba=False), None
        except PermissionError as ex:
            errors.append(str(ex))
            print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: workbook permission denied, attempt {attempt}/3. Trying temp snapshot...", flush=True)
            temp_path = try_filesystem_copy(source_path, sheet_key)
            if not temp_path:
                temp_path = try_excel_save_copy_as(source_path, sheet_key)
            if temp_path:
                try:
                    return load_workbook(temp_path, read_only=True, data_only=True, keep_vba=False), temp_path
                except Exception as temp_ex:
                    errors.append(str(temp_ex))
                    try:
                        os.remove(temp_path)
                    except Exception:
                        pass
            ...
            if getattr(ex, "errno", None) == 13:
                errors.append(str(ex))
                print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] {sheet_key}: workbook locked by Windows/Excel, attempt {attempt}/3. Trying temp snapshot...", flush=True)
                temp_path = try_filesystem_copy(source_path, sheet_key) or try_excel_save_copy_as(source_path, sheet_key)
                if temp_path:
                    try:
                        return load_workbook(temp_path, read_only=True, data_only=True, keep_vba=False), temp_path
                    except Exception as temp_ex:
                ...
                raise
    raise RuntimeError("Workbook is still locked or permission denied after retries. Save/close Excel or install pywin32 for open-workbook snapshot support. Last errors: " + " | ".join(errors[-3:]))
...
def parse_file(sheet_key, path):
    ...
    wb, temp_workbook_path = open_workbook_with_lock_fallback(path, sheet_key)
    try:
