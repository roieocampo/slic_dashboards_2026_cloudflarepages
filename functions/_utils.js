...
function countPartsByCategory(rows, partIdx, colIdx, categories, includeOthers = true) {
  ...
  for (const row of rows) {
    ...
    if (includeOthers && !matched) {
      if (!dynamicOthers.has(value)) dynamicOthers.set(value, new Set());
      dynamicOthers.get(value).add(partKey(row, partIdx));
    }
  }
  const out = Object.fromEntries(Object.entries(counts).map(([k, v]) => [k, v.size]));
  for (const [value, set] of [...dynamicOthers.entries()].sort((a, b) => a[0].localeCompare(b[0]))) out[value] = set.size;
  return out;
}
function countPeUpdates(rows, partIdx, peIdx, finalIdx) {
  ...
  for (const row of rows) {
    ...
    if (!matched && pe) {
      if (!dynamicOthers.has(pe)) dynamicOthers.set(pe, new Set());
      dynamicOthers.get(pe).add(pk);
    }
  }
  const out = Object.fromEntries(Object.entries(counts).map(([k, v]) => [k, v.size]));
  for (const [value, set] of [...dynamicOthers.entries()].sort((a, b) => a[0].localeCompare(b[0]))) out[value] = set.size;
