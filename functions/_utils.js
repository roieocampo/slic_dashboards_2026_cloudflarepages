...
function addDynamicOther(dynamicOthers, rawValue, row, partIdx) {
  ...
  dynamicOthers.get(key).parts.add(partKey(row, partIdx));
}
...
function countPartsByCategory(rows, partIdx, colIdx, categories, includeOthers = true) {
  ...
  for (const row of rows) {
    ...
    for (const cat of categories) {
      if (categoryMatch(value, cat)) {
        counts[cat].add(partKey(row, partIdx));
        matched = true;
      }
    }
    if (includeOthers && !matched) addDynamicOther(dynamicOthers, value, row, partIdx);
  }
  const out = Object.fromEntries(Object.entries(counts).map(([k, v]) => [k, v.size]));
  for (const { label, parts } of [...dynamicOthers.values()].sort((a, b) => a.label.localeCompare(b.label))) out[label] = parts.size;
  return out;
}
...
function countPeUpdates(rows, partIdx, peIdx, finalIdx) {
  ...
  for (const row of rows) {
    ...
    // Completed can come from PE Updates OR Final Remarks / Status.
    // This catches variants such as "RB to SUV converted", "RB-SUV Converted",
    // "SUV HW Checker", extra spaces, punctuation, and small wording differences.
    if (categoryMatch(pe, 'Completed') || categoryMatch(fin, 'Completed')) {
      counts.Completed.add(pk);
      matched = true;
    }

    for (const cat of ['Failed', 'Ongoing', 'Insufficient Data', 'Pending']) {
      if (categoryMatch(pe, cat)) {
        counts[cat].add(pk);
        matched = true;
      }
    }

    if (!matched && pe) addDynamicOther(dynamicOthers, pe, row, partIdx);
  }
  const out = Object.fromEntries(Object.entries(counts).map(([k, v]) => [k, v.size]));
  for (const { label, parts } of [...dynamicOthers.values()].sort((a, b) => a.label.localeCompare(b.label))) out[label] = parts.size;
  return out;
